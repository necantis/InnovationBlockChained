...
idea_challenge_num <- 10 # max ideas per challenge
...
createAgents<- function(agents_num, product_dim, market_dim, minSkill_1, minSkill_2, maxSkills, levels){

  #Creating a temporary matrix with product skills of the agent
  s_pro <- matrix(, nrow = agents_num, ncol = product_dim)
#Creating a temporary matrix with market skills of the agent  
  s_mkt <- matrix(, nrow = agents_num, ncol = market_dim)
...
#Combining results into one dataframe    
  return(data.frame(
    UID=seq(1:agents_num),
    s_pro,
    s_mkt,
    money = rep(0,agents_num)
    )
  )
}
```
...
To create agents participating to the idea challenges, the system takes as initial parameters the whole set of possible market dimensions assessed by the client (*market_dim*=`r market_dim`), the whole set of possible market dimensions assessed by the client (*product_dim*=`r product_dim`), the maximum skill level per dimension (*levels*=`r levels`), the number of participants (*participants_num*=`r participants_num`).  

```{r participants_Table}

participants <- createAgents(participants_num, product_dim, market_dim, 0, 0, levels, levels)  
colnames(participants)[1]<-"Part_ID"
...
```{r experts_Table}
experts <- createAgents(experts_num, product_dim, market_dim, expert_skill, 0, levels, levels)
...
To create agents as clients, the system he number of clients (*clients_num*=`r clients_num`), and it sets the minimum skill for *business* dimensions at `r expert_skill`/`r levels`.  
 
```{r clients_Table}
clients <- createAgents(clients_num, product_dim, market_dim, 0, expert_skill, levels, levels)
colnames(clients)[1]<-"Client_ID"
clients%>%
     kable()
```

-	phase 01 creates an idea challenge by assigning ideas to a random pool of participants (one participant can have multiple ideas in the same idea challenge).
```{r ideas_Table}
# getIdeas <- function(participants,ideas_num,product_dim, market_dim, levels){
# 
# participants_num <- dim(participants)[1]

# Assigning the ideas to a random pool of participants
# By using Replace, a participant can have more than one idea
Participant_ID <- data.frame("Part_ID"=sample(participants$Part_ID, idea_challenge_num, replace=TRUE))

#The skill of an idea cannot be greater than its creator's skill
skills <- left_join(Participant_ID, participants, by ="Part_ID")
...
ideas <- data.frame(Participant_ID, createAgents(idea_challenge_num, product_dim, market_dim, 0,0, skills, levels))
...
- In phase 02, a client and `r expert_challenge_num` experts assess the ideas of the participants. 
...
```{r assessment_Table}
ideas_scores <- data.frame(matrix(, nrow = dim(ideas)[1], ncol = dim(ideas)[2]-2))
colnames(ideas_scores) <- colnames(ideas[,3:dim(ideas)[2]])
...
for(i in 1:dim(ideas)[1]){
  ideas_scores[i,] <- pmin(ideas[i,-1][,-1],selected_client[,-1])
}  
...
#Adding a set of random experts
for(j in 1:expert_challenge_num){
  for(i in 1:dim(ideas)[1])
  ideas_scores[i,] <- ideas_scores[i,] + pmin(ideas[i,-1][,-1],selected_experts[j,-1])
}

colnames(selected_client)[1] <- "Agent_ID"
colnames(selected_experts)[1] <- "Agent_ID"
assessment_team <- selected_client%>%
  union(selected_experts)

kable(assessment_team) 

# Making the average of calculated scores
final_score <- rowMeans(ideas_scores[,-dim(ideas_scores)[2]])
ideas_scores <- data.frame(ideas[,1:2],round(ideas_scores/(1+expert_challenge_num)),final_score)
...
-	phase 03 rewards the best idea and the owner of the best idea. 
...
*Under the assumption of creative-destruction, the scores of the winning idea are trasfered to all participants of the challenge, making it harder for the winner to win again with the same idea.*  
