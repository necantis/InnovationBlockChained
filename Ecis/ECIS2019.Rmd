...
market_dim <- 5 # skill types concerning the market
product_dim <- 5 # skill types concerning the product
levels <- 100 # Setting the maximum skills per dimension

challenges_num <- 100 # how many iterations done
idea_challenge_num <- 10 # how many ideas per challenge


```
-	phase 00 generates the agents and it assigns one or more ideas to each agent. It takes as initial parameters the whole set of possible market dimensions assessed by the client (*market_dim*=`r market_dim`), the whole set of possible market dimensions assessed by the client (*product_dim*=`r product_dim`), the number of clients (*clients_num*=`r clients_num`), the number of experts (=`r experts_num`), the number of agents (*agents_num*=`r agents_num`), the maximum skill level per dimension (*levels*=`r levels`) and the number of challenges (*challenges_num*=`r challenges_num`). 
...
#Assigning numbers from uniform distribution to agents' product skills set  
  ...
  if(is.data.frame(maxSkills)){
    ...
    s_pro[,i] <- as.integer(runif(agents_num,
                                  min=minSkill_1, 
                                  maxSkills[,i]
                                  )
                            )
...
#Assigning numbers from uniform distribution to agents' market skills set  
  ...
  if(is.data.frame(maxSkills)){
    #Use the skill sets as maximum numbers
    s_mkt[,i] <- as.integer(runif(agents_num,
                                  min=minSkill_2, 
...
agents <- createAgents(agents_num, product_dim, market_dim, 0, 0, levels, levels)  
colnames(agents)[1]<-"Agent_ID"
head(agents)

experts <- createAgents(experts_num, product_dim, market_dim, 50, 0, levels, levels)
colnames(experts)[1]<-"Expert_ID"
# head(experts)

clients <- createAgents(clients_num, product_dim, market_dim, 0, 50, levels, levels)
...
```{r ideas_Table}
#Assigning ideas to agents
Agent_ID <- data.frame("Agent_ID"=sample(agents$Agent_ID, ideas_num, replace=TRUE))
#The skill of an idea cannot be greater than its agent's skill
skills <- left_join(Agent_ID, agents, by ="Agent_ID")
...
ideas <- data.frame(Agent_ID, createAgents(ideas_num, product_dim, market_dim, 0,0, skills, levels))
colnames(ideas)[2] <- "Idea_ID"
head(ideas)


#Assigning ideas to challenges
Idea_ID <- data.frame("Idea_ID"=sample(ideas$Agent_ID, ideas_num, replace=TRUE))


Expert_ID <- data.frame("Expert_ID"=sample(experts$Expert_ID, ideas_num, replace=TRUE))
